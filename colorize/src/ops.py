...
def pixelwise_accuracy(img_real, img_fake, colorspace, thresh):
    ...
    diffL = tf.compat.v1.abs(tf.compat.v1.round(img_real[..., 0]) - tf.compat.v1.round(img_fake[..., 0]))
    diffA = tf.compat.v1.abs(tf.compat.v1.round(img_real[..., 1]) - tf.compat.v1.round(img_fake[..., 1]))
    diffB = tf.compat.v1.abs(tf.compat.v1.round(img_real[..., 2]) - tf.compat.v1.round(img_fake[..., 2]))

    # within %thresh of the original
    predL = tf.compat.v1.cast(tf.compat.v1.less_equal(diffL, 1 * thresh), tf.compat.v1.float64)        # L: [0, 100]
    predA = tf.compat.v1.cast(tf.compat.v1.less_equal(diffA, 2.2 * thresh), tf.compat.v1.float64)      # A: [-110, 110]
    predB = tf.compat.v1.cast(tf.compat.v1.less_equal(diffB, 2.2 * thresh), tf.compat.v1.float64)      # B: [-110, 110]

    # all three channels are within the threshold
    pred = predL * predA * predB

    return tf.compat.v1.reduce_mean(pred)
...
def preprocess(img, colorspace_in, colorspace_out):
    if colorspace_out.upper() == COLORSPACE_RGB:
        ...
        L_chan, a_chan, b_chan = tf.compat.v1.unstack(img, axis=3)

        # L: [0, 100] => [-1, 1]
        # A, B: [-110, 110] => [-1, 1]
        img = tf.compat.v1.stack([L_chan / 50 - 1, a_chan / 110, b_chan / 110], axis=3)
...
def postprocess(img, colorspace_in, colorspace_out):
    if colorspace_in.upper() == COLORSPACE_RGB:
        ...
        # L: [-1, 1] => [0, 100]
        # A, B: [-1, 1] => [-110, 110]
        img = tf.compat.v1.stack([(L_chan + 1) / 2 * 100, a_chan * 110, b_chan * 110], axis=3)
...
def rgb_to_lab(srgb):
    ...
    with tf.compat.v1.name_scope("rgb_to_lab"):
        srgb_pixels = tf.compat.v1.reshape(srgb, [-1, 3])
        ...
        return tf.compat.v1.reshape(lab_pixels, tf.compat.v1.shape(srgb))


def lab_to_rgb(lab):
    with tf.compat.v1.name_scope("lab_to_rgb"):
        lab_pixels = tf.compat.v1.reshape(lab, [-1, 3])
