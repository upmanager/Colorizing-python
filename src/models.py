...
class BaseModel:
    ...
    def test(self):
        ...
        for index in range(len(dataset)):
            ...
            feed_dic = {self.input_gray: img_gray}
            # , feed_dict=feed_dic
            outputs = self.sess.run(self.sampler)
    ...
    def sample(self, show=True):
        ...
        fake_image = postprocess(tf.compat.v1.convert_to_tensor(fake_image), colorspace_in=self.options.color_space, colorspace_out=COLORSPACE_RGB)
    ...
    def build(self):
        ...
        self.input_rgb = tf.compat.v1.placeholder(tf.compat.v1.float32, shape=(None, None, None, 3), name='input_rgb')
        ...
        # train/turing-test we extract grayscale image from color image
            ...
            self.input_gray = tf.compat.v1.image.rgb_to_grayscale(self.input_rgb)

        gen = gen_factory.create(self.input_gray, kernel, seed)
        dis_real = dis_factory.create(tf.compat.v1.concat([self.input_gray, self.input_color], 3), kernel, seed)
        dis_fake = dis_factory.create(tf.compat.v1.concat([self.input_gray, gen], 3), kernel, seed, reuse_variables=True)

        gen_ce = tf.compat.v1.nn.sigmoid_cross_entropy_with_logits(logits=dis_fake, labels=tf.compat.v1.ones_like(dis_fake))
        dis_real_ce = tf.compat.v1.nn.sigmoid_cross_entropy_with_logits(logits=dis_real, labels=tf.compat.v1.ones_like(dis_real) * smoothing)
        dis_fake_ce = tf.compat.v1.nn.sigmoid_cross_entropy_with_logits(logits=dis_fake, labels=tf.compat.v1.zeros_like(dis_fake))

        self.dis_loss_real = tf.compat.v1.reduce_mean(dis_real_ce)
        self.dis_loss_fake = tf.compat.v1.reduce_mean(dis_fake_ce)
        self.dis_loss = tf.compat.v1.reduce_mean(dis_real_ce + dis_fake_ce)

        self.gen_loss_gan = tf.compat.v1.reduce_mean(gen_ce)
        self.gen_loss_l1 = tf.compat.v1.reduce_mean(tf.compat.v1.abs(self.input_color - gen)) * self.options.l1_weight
        self.gen_loss = self.gen_loss_gan + self.gen_loss_l1

        self.sampler = tf.compat.v1.identity(gen_factory.create(self.input_gray, kernel, seed, reuse_variables=True), name='output')
        self.accuracy = pixelwise_accuracy(self.input_color, gen, self.options.color_space, self.options.acc_thresh)
        self.learning_rate = tf.compat.v1.constant(self.options.lr)

        # learning rate decay
        if self.options.lr_decay and self.options.lr_decay_rate > 0:
            self.learning_rate = tf.compat.v1.maximum(1e-6, tf.compat.v1.train.exponential_decay(
